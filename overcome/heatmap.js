    ...
    let promise = await new Promise((resolve) => {
        ...
        reader.onload = function () {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                var latLngAddr = { lat: parseFloat(rowContent[0]), lng: parseFloat(rowContent[1]), address: rowContent[3] };
                var hmValue = parseInt(rowContent[2])       // heatmap value to determine color
                if (!isNaN(latLngAddr.lat) && !isNaN(latLngAddr.lng) && !isNaN(hmValue)) { // headers 'Lat'/'Lon' => parseFloat == NaN, don't push it.
                    uecoords.push(latLngAddr);
                    rsrp.push(hmValue)
    ...
    for (i = 0; i < uecoords.length; i++) {
        ...
        if (google.maps.geometry.poly.containsLocation(ueLatLng, fruitBelt) == true) {
            ...
            marker.content = `<div id="infoWindow">
                                <b>Address:</b> ${uecoords[i].address}<br />
